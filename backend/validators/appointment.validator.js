...
export const AppointmentValidator = {
  validateCreate(data) {
    ...
    if (data.total_duration != null && !Number.isInteger(data.total_duration) || data.total_duration <= 0) {
      throw new Error(
    ...
    if (dt.getTime() > max.getTime()) { // Verificar primero si vale la pena limitar por meses, y si manejar MM/DD/AAAA o DD/MM/AAAA
      throw new Error(`appointment_datetime must be within the next ${MAX_MONTH} months`);
    }

    if (!Array.isArray(data.services_ids) || data.services_ids.length === 0) { // check int
  ...
  validateFiltersListAppointments(filters){
    ...
    const barberId = filters.barber_id != null ? Number(filters.barber_id) : null;
    ...
    if(sort != null && !orders.includes(sort)){
      throw new Error(`Invalid status value. Must be one of: ${orders.join(", ")}`);
    }
    ...
    if(filters.status != null && !states.includes(filters.status)) {
      throw new Error(`Invalid status value. Must be one of: ${states.join(", ")}`);
    }

    if(filters.orderValue != null && orderValue != null && !orderValues.includes(orderValue)){
      throw new Error(`Invalid order value. Must be one of: ${orderValues.join(", ")}`)
    }
